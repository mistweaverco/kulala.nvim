...
if not pandoc then _G.pandoc = { utils = {} } end

-- if there's no pandoc.utils, create a local one
if not pcall(require, "pandoc.utils") then pandoc.utils = {} end
...
if not pandoc.utils.type then
  pandoc.utils.type = function(value)
    ...
    elseif value.tag and value.t then
      typ = value.tag
      if typ:match("^Meta.") then typ = typ:sub(5) end
      if typ == "Map" then typ = "table" end
    end
    return typ
  end
end
...
local function dump_(prefix, value, maxlen, level, add)
  ...
  if add == nil then add = function(item)
...
logging.dump = function(value, maxlen)
  ...
  if #text > maxlen then text = dump_(nil, value, nil) end
  return text
end
...
logging.output = function(...)
  ...
  if need_newline then io.stderr:write("\n") end
end
...
logging.error = function(...)
  if logging.loglevel >= -1 then logging.output("(E)", ...) end
end

logging.warning = function(...)
  if logging.loglevel >= 0 then logging.output("(W)", ...) end
end

logging.info = function(...)
  if logging.loglevel >= 1 then logging.output("(I)", ...) end
end

logging.debug = function(...)
  if logging.loglevel >= 2 then logging.output("(D)", ...) end
end
...
logging.trace = function(...)
  if logging.loglevel >= 3 then logging.output("(T)", ...) end
end
