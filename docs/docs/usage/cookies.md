...
By default, Kulala will store incoming cookies in a cookie jar, which is a file named `cookie.txt` in Kulala's cache directory. 
You can open the file with the keymap `<leader>Rj` or by using the relevant code action.

The same file is used with `@attach-cookie-jar` metadata to attach cookies to requests.
