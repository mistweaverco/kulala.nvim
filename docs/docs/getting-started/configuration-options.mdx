...
Path to the [kulala-core](https://github.com/mistweaverco/kulala-core) executable.

When set, this path is used exclusively. When `nil` (default), Kulala resolves `kulala-core` from your `PATH`.
...
```lua
{
  ...
  opts = {
    kulala_core_path = "/home/bonobo/.local/bin/kulala-core",
  },
}
```
...
Subprocess timeout (milliseconds) for kulala-core invocations.

Default: `600000` (10 minutes). Set to `nil` to disable the Neovim-side timeout (not recommended).
...
```lua
{
  "mistweaverco/kulala.nvim",
  opts = {
    kulala_core_timeout = 120000,
  },
}
```

### kulala_core_data_dir

Override directory for kulala-core persistence (cookies, OAuth tokens, prompts). Defaults to the same path as the kulala-core CLI.
